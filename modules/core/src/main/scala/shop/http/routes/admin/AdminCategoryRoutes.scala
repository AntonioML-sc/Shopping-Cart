...
final class AdminCategoryRoutes[F[_] : Defer : Monad: JsonDecoder](categories: Categories[F])
                                                                  (implicit ME: MonadError[F, Throwable]) extends Http4sDsl[F] {
  private[admin] val prefixPath: String = "/categories"
  private val httpRoutes: HttpRoutes[F] = HttpRoutes.of[F] {
    case ar @ POST -> Root => ar.asJsonDecode[CategoryParam].flatMap { cp =>
...
object AdminCategoryRoutes {
  def apply[F[_]: Defer: Monad: JsonDecoder](categories: Categories[F])(implicit ME: MonadError[F, Throwable]): AdminCategoryRoutes[F] =
