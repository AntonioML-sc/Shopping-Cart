...
import shop.domain.brand.{ BrandId, BrandName, BrandParam, OldBrandName, RenameBrandInfo }
...
final class AdminBrandRoutes[F[_]: Defer: Monad: JsonDecoder](brands: Brands[F])(implicit ME: MonadError[F, Throwable])
  ...
  private val httpRoutes: HttpRoutes[F] = HttpRoutes.of[F] {
    case ar @ POST -> Root                 => ar.asJsonDecode[BrandParam].flatMap(bp => Created(brands.create(bp.toDomain)))
